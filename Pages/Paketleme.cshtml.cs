...
public sealed class PaketlemeModel(SusamRepository repository):PageModel
{
    ...
    private async Task LoadAsync(){try{Records=await repository.GetPaketlemeAsync(filter:Filter,personnelId:IsAdmin?null:HttpContext.PersonnelId());Personeller=await repository.GetPersonellerAsync();Menseiler=await repository.GetMenseilerAsync();Urunler=await repository.GetUrunlerAsync();}catch(Exception ex){ErrorMessage=ex.Message;}}
}
