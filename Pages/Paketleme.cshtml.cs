...
public sealed class PaketlemeModel(SusamRepository repository):PageModel
{
    ...
    private async Task LoadAsync(){try{if(IsAdmin)Records=await repository.GetPaketlemeAsync(filter:Filter);Personeller=await repository.GetPersonellerAsync();Menseiler=await repository.GetMenseilerAsync();Urunler=await repository.GetUrunlerAsync();}catch(Exception ex){ErrorMessage=ex.Message;}}
}
