...
public sealed class DolumModel(SusamRepository repository,ExcelExportService excel):PageModel
{
    ...
    private async Task LoadAsync(){try{if(IsAdmin)Records=await repository.GetDolumAsync(filter:Filter);Ambalajlar=await repository.GetAmbalajlarAsync();Urunler=excel.FilterProducts(await repository.GetUrunlerAsync());Personeller=await repository.GetPersonellerAsync();}catch(Exception ex){ErrorMessage=ex.Message;}}
}
