...
{
    [BindProperty]public DolumInput Input{get;set;}=new();[BindProperty(SupportsGet=true)]public RecordFilter Filter{get;set;}=new();[BindProperty(SupportsGet=true)]public long? EditId{get;set;}public bool IsAdmin=>HttpContext.IsAdmin();public string? PersonnelName=>HttpContext.PersonnelName();public List<DolumListItem>Records{get;private set;}=[];public List<LookupItem>Ambalajlar{get;private set;}=[];public List<LookupItem>Menseiler{get;private set;}=[];public List<LookupItem>Personeller{get;private set;}=[];public string?ErrorMessage{get;private set;}
    ...
    private async Task LoadAsync(){try{Records=await repository.GetDolumAsync(filter:Filter,personnelId:IsAdmin?null:HttpContext.PersonnelId());Ambalajlar=await repository.GetAmbalajlarAsync();Menseiler=await repository.GetMenseilerAsync();Personeller=await repository.GetPersonellerAsync();}catch(Exception ex){ErrorMessage=ex.Message;}}
}
