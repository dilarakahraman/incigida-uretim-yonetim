...
public sealed class DolumModel(SusamRepository repository):PageModel
{
    ...
    private async Task LoadAsync(){try{Records=await repository.GetDolumAsync(filter:Filter,personnelId:IsAdmin?null:HttpContext.PersonnelId());Ambalajlar=await repository.GetAmbalajlarAsync();Urunler=await repository.GetUrunlerAsync();Personeller=await repository.GetPersonellerAsync();}catch(Exception ex){ErrorMessage=ex.Message;}}
}
